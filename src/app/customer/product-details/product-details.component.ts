...
export class ProductDetailsComponent implements OnInit{
  ...
  public productId: string | number | undefined
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe((params) => {
      ...
      if (typeof this.productId === 'string' || typeof this.productId === 'number') {
        this.shopService.getProduct(this.productId?.toString()).subscribe((res) => {
          if (res) {
    ...
    if(this.productId){
      ...
      if(!user){
        if (typeof this.productId === 'string' || typeof this.productId === 'number') {
          ...
          this.shopService.removeFromLocal(this.productId);
        }
        this.removeCartLink = false
      }else{
        // console.log(this.productId);
        this.shopService.removeItemFromCart(this.productId?.toString()).subscribe((res) => {
        if(res){
          this.shopService.getCartCount()
          this.removeCartLink = false
        }
